...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Kawan Jalan'); ?></title>
    ...
    <?php if(file_exists(public_path('build/manifest.json')) || app()->environment('local')): ?>
        <?php echo app('Illuminate\Foundation\Vite')(['resources/css/app.css', 'resources/js/app.js']); ?>
    <?php endif; ?>

    <?php if(!file_exists(public_path('build/manifest.json'))): ?>
        <style>
            <?php echo file_exists(resource_path('css/app.css')) ? file_get_contents(resource_path('css/app.css')) : ''; ?>
...
<body>
    <div class="app-shell">
        <?php if(session('success')): ?>
            <div class="alert toast-notify show"><?php echo e(session('success')); ?></div>
        <?php endif; ?>

        <?php if(session('danger')): ?>
            <div class="alert error toast-notify show"><?php echo e(session('danger')); ?></div>
        <?php endif; ?>

        <?php if(auth()->guard()->check()): ?>
            <?php if(auth()->user()->status === 'banned'): ?>
                <div class="suspended-banner">Akun anda telah tersuspend, hubungi kontak kami kelompok6@gmail.com</div>
            <?php endif; ?>
        <?php endif; ?>

        <?php if($errors->any()): ?>
            <div class="alert error toast-notify show"><?php echo e($errors->first()); ?></div>
        <?php endif; ?>
...
<?php /**PATH C:\laragon\www\SMS 4 PROJECT\HAMPIR BERESS\10 fINAL FIX\Kawan Jalan web\resources\views/layouts/app.blade.php ENDPATH**/ ?>